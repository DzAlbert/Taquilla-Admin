...
import { Plus, Calendar, PencilSimpleLine, X, MagnifyingGlass, CheckCircle, XCircle, Warning, Clock, SunHorizon, Moon } from '@phosphor-icons/react'
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-lg font-medium">
              {search || statusFilter !== 'all' ? 'No se encontraron sorteos' : 'No hay sorteos registrados'}
            </p>
            <p className="text-muted-foreground text-sm mb-4">
              {search || statusFilter !== 'all' ? 'Intenta con otros criterios de búsqueda' : 'Crea tu primer sorteo para comenzar'}
            </p>
            {!search && statusFilter === 'all' && (
              <Button onClick={() => setLotteryDialogOpen(true)} variant="outline" className="gap-2">
                <Plus weight="bold" />
                Crear Primer Sorteo
              </Button>
            )}
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {filteredLotteries.map((lottery) => (
            <Card
              ...
              style={{
                borderLeftColor: lottery.isActive ? 'rgb(16 185 129)' : 'rgb(156 163 175)'
              }}
            >
              <CardContent className="px-4 py-2">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>Abre: <span className="font-medium text-foreground">{lottery.openingTime}</span></span>
                  </div>

                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Moon className="h-3.5 w-3.5 shrink-0" />
                    <span>Cierra: <span className="font-medium text-foreground">{lottery.closingTime}</span></span>
                  </div>

                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Clock className="h-3.5 w-3.5 shrink-0" />
                    <span>Jugada: <span className="font-medium text-foreground">{lottery.drawTime}</span></span>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              ¿Está seguro que desea eliminar el sorteo <span className="font-semibold text-foreground">"{lotteryToDelete?.name}"</span>?
            </p>
          </div>
